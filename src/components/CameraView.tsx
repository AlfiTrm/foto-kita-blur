...
export function CameraView({
  ...
  const stageAspectRatio = getStageAspectRatio(frameId)
  const hasFrame = frameId !== 'none'

  return (
    <section className="relative mx-auto w-full" style={{ aspectRatio: stageAspectRatio }}>
      <div
        className={`absolute inset-0 overflow-hidden rounded-[30px] bg-[#dfe5ef] ${
          hasFrame
            ? ''
            : 'border border-white/80 shadow-[0_28px_90px_rgba(15,23,42,0.12)]'
        }`}
      >
        {photoDataUrl ? (
          <img
      ...
      {hasFrame ? (
        <>
          <div className="pointer-events-none absolute inset-0 rounded-[34px] shadow-[0_28px_90px_rgba(15,23,42,0.14)]" />
          {renderFrameOverlay(frameId, frameOverlayRef)}
        </>
      ) : null}
    </section>
  )
